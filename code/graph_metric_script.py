...
def node_distribution(posdf, lam1=.1):
    total_sum = 0
    for (i, row_i), (j, row_j) in itertools.combinations(posdf.iterrows(), 2):
        ...
        if dij != 0:  # Avoid division by zero
            total_sum += lam1 / dij**2
    return total_sum

# DISTANCE TO BORDERLINES

def distance_to_borderlines(posdf, lam2=.001):
    total_sum = 0
    ...
    return round(total_sum*lam2, 3)


# EDGE LENGTHS SUM

def edge_length_sum(graph, posdf, lam3=.01):
    total_length = 0
...
def g_visualisation_metric(G, pos, lam1=.1, lam2=.001, lam3=.01, lam5=.1):
    ...
    pen1 = node_distribution(posdf, lam1)
    pen2 = distance_to_borderlines(posdf, lam2)
    pen3 = edge_length_sum(G, posdf, lam3)
    (pen5, lam4) = edge_node_distance_contribution(G, posdf, lam5)
    pen4 = count_edge_crossings(G, posdf, lam4)
